...
function searchJira() {
    ...
    for (i ; i < j; i++) {
        ...
        console.log('**i*' + i);
        request(`https://finastra.atlassian.net/rest/api/3/search?jql=type=Story&startAt=${startAt}`, options, function(error, response, body) {
            ...
            if (temp.issues.length > 0) {
                ...
                console.log('888' + jiraExtIdMap);
				checkIfExistingOrNew(jiraExtIdMap);
...
function checkIfExistingOrNew(jiraExtIdMap) {
    //console.log('**jiraExtIdMap**' + jiraExtIdMap.get('a0n9E00000307oRQAQ'));
    //for (let sfKey of sfIdMap.keys()) {
    for (let sfKey of jiraExtIdMap.keys()) {
		
		//console.log('**jiraExtIdMap**' + jiraExtIdMap.get(sfKey));
        if (jiraExtIdMap.get(sfKey) === null || jiraExtIdMap.get(sfKey) === undefined) {
